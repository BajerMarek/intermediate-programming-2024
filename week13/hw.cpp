...
std::set<std::string> employees_with_missing_records(std::vector<std::tuple<std::string, int, bool>> records) {
    ...
    std::map<std::string,std::set<bool>> seen;

    for(int i = 0; i<records.size();i++)    //?projde kazdý vector
    {
        auto [id, cas, typ_zaznamu] = records[i];
        if(!(seen.contains(id))&&(typ_zaznamu==true))
        {
            seen.insert({id,{typ_zaznamu}});
        }
        else
        {        
            if((typ_zaznamu==true)&&!(seen[id].contains(true)))
            {
                seen.insert({id,{typ_zaznamu}});
            }
            else 
            {
                if((seen[id].contains(true))&&(typ_zaznamu==false))
                {
                    //std::cout<<"tolikata iterace: "<<i<<" s tímto id: "<<id<<std::endl;
                    seen.erase(id);
                }
                else
                {
    ...
    // kod zamestnance 
    //todo PRAVIDLA:
    //_ odlešly dřív ne přišly          #
    //_ přišly ale neodešly             #
    //_ odešly ale nepřišly             #
    //_ přišly vcýkrát než odšly        #
    //_ odešly výckrát než přišly       #

    //! =>

    //? počet příchodů = počet odchdů
    //? příchod musí být dřív určený odchod     -> kdyz to de zasebou stačí říct jestly uz má napsaný příchod
    //?                                            jetly ne tak je to automaticky spatne
